...
class GoogleGetToken extends Command
{
    ...
    public function handle()
    {
        ...
        if (file_exists($credentialsPath)
            ...
            && !$this->confirm('Có muốn làm mới token không?')
        ) {
            //nếu không tồn tại file token thì trả ra thông báo
            return $this->info('Không tồn tại file token');
        }
    ...
    private function getClient()
    {
        ...
        $client->setApplicationName('Medicine'); // đặt tên cho ứng dụng (không quan trọng lắm)
        $configPath = $this->addTrailingSlash(getcwd());
        ...
        $client->setAuthConfig( $configPath . '/storage/google/credentials.json' ); // đường dẫn đến file credentials.json
